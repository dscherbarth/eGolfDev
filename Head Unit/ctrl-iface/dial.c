...
gtk_dial_init (GtkDial *dial)
{
  ...
  dial->gf_tick =  gdk_font_load("-*-helvetica-bold-r-*-*-*-160-*-*-*-*-*-*");
  dial->gf_ddisp =  gdk_font_load("-*-helvetica-bold-r-*-*-*-240-*-*-*-*-*-*");
...
void gtk_dial_set_tag (GtkDial *dial, char  *tag)
{
	strcpy (dial->tag, tag);
}

void gtk_dial_set_redline (GtkDial *dial, int  redline)
{
	dial->redline = redline;
}
GdkColor grayl = {0, 0xffff, 0x0000, 0x0000};
GdkColor blackl = {0, 0x0000, 0x0000, 0x0000};
GdkColor whitel = {0, 0xefff, 0xefff, 0xefff};
...
*/
  ...
  xa = widget->allocation.width;
  ya = widget->allocation.height;

  sprintf(temp, "%6.0f", (float)dial->adjustment->value);
  gdk_draw_string (widget->window,
				dial->gf_ddisp,
				widget->style->fg_gc[widget->state],
				xa - 140, ya - 45,
				temp);
  sprintf(temp, "%6.0f", (float)dial->adjustment->lower);
  gdk_draw_string (widget->window,
				dial->gf_tick,
				widget->style->fg_gc[widget->state],
				5, ya - 30,
				temp);
  sprintf(temp, "%6.0f", (float)dial->adjustment->upper);
  gdk_draw_string (widget->window,
				dial->gf_tick,
				widget->style->fg_gc[widget->state],
				xa - 55, ya - 30,
